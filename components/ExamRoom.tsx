...
export default function ExamRoom({ questions, studentInfo, duration, onFinish }: ExamRoomProps) {
  ...
  if (!questions || questions.length === 0) return <div className="p-10 text-white">Đang tải câu hỏi...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-4 pb-40">
      ...
      <div className="sticky top-0 z-50 bg-slate-900/90 backdrop-blur-xl border-b-2 border-emerald-500/30 p-4 mb-8 flex justify-between items-center rounded-3xl shadow-2xl">
        ...
        <button onClick={() => onFinish()} className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-2xl font-black">NỘP BÀI</button>
      </div>
          ...
          <div key={idx} className="bg-slate-900 border-2 border-slate-800 p-6 md:p-10 rounded-[2.5rem] shadow-xl mb-10">
            ...
            {q.type === 'true-false' && q.s && (
              <div className="space-y-3">
                {q.s.map((sub, sIdx) => (
                  <div key={sIdx} className="flex items-center justify-between p-4 border border-slate-800 rounded-2xl bg-slate-800/30">
                    <div className="flex-1 pr-4 text-slate-200">
                    ...
                    <div className="flex gap-2">
                      {['Đúng', 'Sai'].map((label) => {
                        ...
                        const isSelected = Array.isArray(answers[idx]) && answers[idx][sIdx] === val;
                        return (
                          <button
                            key={label}
                            onClick={() => handleAnswerChange(idx, val, sIdx)}
                            className={`px-4 py-2 rounded-xl font-bold border-2 transition-all ${isSelected ? 'bg-emerald-600 border-emerald-500 text-white' : 'bg-slate-700 border-slate-600 text-slate-400'}`}
                          >
                            {label}
                          </button>
                        );
            ...
            {(q.type === 'short-answer' || q.type === 'sa') && (
              <div className="mt-4 p-6 bg-slate-800/50 rounded-[2rem] border-2 border-slate-700 flex items-center gap-4">
                ...
                <input
                  type="text"
                  className="flex-1 bg-slate-900 border-2 border-slate-700 p-4 rounded-xl text-white font-bold focus:border-emerald-500 outline-none"
